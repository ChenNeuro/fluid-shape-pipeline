...
macro_f1 = float(
    np.mean([
        ...
        for i in range(n_shapes) if np.any(shape_idx_test == i)
    ])
)
print(f"Accuracy: {accuracy:.1%} | Macro-F1: {macro_f1:.1%} | dy_MAE: {dy_mae:.5f} | eps_MAE: {eps_mae:.5f}")

# ── colormaps ───────────────────────────────────────────────────────────────
def vorticity_cmap():
    colors = ["#08306b", "#2171b5", "#6baed6", "#c6dbef", "#f7fbff",
              "#fff5f0", "#fee0d2", "#fc9272", "#de2d26", "#67000d"]
...
SHAPE_COLORS = {
    "circle": "#60a5fa", "triangle": "#f97316",
    "airfoil": "#a78bfa", "diamond": "#34d399", "bar": "#fbbf24",
}
...
for shi, shape in enumerate(shape_labels_ordered):
    ...
    for si, scale in enumerate(scales):
        ...
        ax.imshow(img, cmap=CHANNEL_CMAPS[ci_vort],
                  vmin=-vabs, vmax=vabs,
        ...
        # Labels
        if shi == 0:
            ax.set_title(SCALE_LABELS.get(scale, scale),
                        color=FG, fontsize=11, fontweight="bold", pad=4)
        if si == 0:
            ax.set_ylabel(shape.capitalize(), color=SHAPE_COLORS.get(shape, FG),
...
fig.text(0.26, 0.97, "A. Vorticity Fields — 3 Downstream Distances",
         color=FG, fontsize=13, fontweight="bold", ha="center", va="top")

# ── Panel B: Confusion Matrix ───────────────────────────────────────────────
ax_b.set_facecolor("#161b22")
cm = confusion_matrix(shape_idx_test, shape_pred_idx,
                      labels=list(range(n_shapes)))
...
for shape in shape_labels_ordered:
    mask = shapes_test == shape
    ax_c.scatter(dy_test[mask], dy_pred[mask],
                 color=SHAPE_COLORS.get(shape, FG), alpha=0.75, s=60,
                 edgecolors="none")
diag = np.linspace(pert_min, pert_max, 200)
ax_c.plot(diag, diag, color="#f85149", linewidth=1.8, linestyle="--",
         label="Perfect", zorder=0)
ax_c.set_xlabel("True dy", color=FG, fontsize=10)
ax_c.set_ylabel("Predicted dy", color=FG, fontsize=10)
ax_c.set_title(f"C. dy Prediction\nMAE={dy_mae:.4f}", color=FG,
...
ax_e.text(n_shapes - 0.5, accuracy + 0.03,
         f"Overall\n{accuracy:.0%}", color="#f85149",
         fontsize=8, ha="center", va="bottom")
ax_e.set_ylim(0, 1.18)
ax_e.set_ylabel("Accuracy", color=FG, fontsize=10)
ax_e.set_title("E. Per-Shape Accuracy", color=FG,
               fontsize=11, fontweight="bold")
...
y = 0.95
ax_legend.text(0.5, y, "Shape Legend", ha="center", va="top",
               color=FG, fontsize=11, fontweight="bold")
y -= 0.10
for shape in shape_labels_ordered:
    color = SHAPE_COLORS.get(shape, FG)
    ax_legend.text(0.05, y, "●", color=color, fontsize=14, va="center", transform=ax_legend.transAxes)
    ax_legend.text(0.15, y, shape.capitalize(), color=FG, fontsize=10, va="center", transform=ax_legend.transAxes)
    y -= 0.12

y -= 0.08
ax_legend.text(0.5, y, "Per-Re Accuracy", ha="center", va="top",
               color=FG, fontsize=11, fontweight="bold")
y -= 0.10
for re_val, acc in re_acc.items():
    ax_legend.text(0.05, y, f"Re={re_val}:", color=TEXT_SEC, fontsize=10, va="center", transform=ax_legend.transAxes)
    ax_legend.text(0.55, y, f"{acc:.0%}", color=FG, fontsize=10, fontweight="bold", va="center", transform=ax_legend.transAxes)
    y -= 0.12
