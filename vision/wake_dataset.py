...
def load_wake_bundle() -> WakeBundle:
    ...
    if not index_path.exists():
        raise FileNotFoundError(f"Missing wake-field index: {index_path}. Run build_wake_fields first.")
    ...
    for _, row in index_df.iterrows():
        ...
        if scale_names is None:
            ...
            raise RuntimeError(f"Inconsistent scale names for case {row['case_id']}: {case_scales} != {scale_names}")

        if channel_names is None:
            channel_names = case_channels
        elif case_channels != channel_names:
            raise RuntimeError(f"Inconsistent channel names for case {row['case_id']}: {case_channels} != {channel_names}")
    ...
    stacked = {scale: np.stack(items, axis=0).astype(np.float32) for scale, items in crops_by_scale.items()}
    return WakeBundle(
...
def variant_tensor(bundle: WakeBundle, *, scales: list[str], channels: list[str]) -> np.ndarray:
    ...
    stacked = np.stack([bundle.crops_by_scale[scale][:, channel_idx, :, :] for scale in scales], axis=1)
